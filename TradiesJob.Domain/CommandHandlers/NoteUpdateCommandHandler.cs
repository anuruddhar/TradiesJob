...
namespace TradiesJob.Domain.CommandHandlers {
    ...
    public sealed class NoteUpdateCommandHandler : ICommandHandler<NoteUpdateCommand, AppResult> {

        private readonly IDatabase _database;
        public NoteUpdateCommandHandler(IDatabase database) {
            _database = database;
        }


        public async Task<AppResult> Handle(NoteUpdateCommand command) {
            ...
            var result = await _database.ExecuteNonQueryAsync<int>(SPConstant.UPDATE_NOTE, param);
            if (result.Item1 >= 1) {
