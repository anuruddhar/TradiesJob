...
namespace TradiesJob.Domain.CommandHandlers {
    ...
    public sealed class NoteCreateCommandHandler : ICommandHandler<NoteCreateCommand, AppResult> {

        private readonly IDatabase _database;
        public NoteCreateCommandHandler(IDatabase database) {
            _database = database;
        }

        public async Task<AppResult> Handle(NoteCreateCommand command) {
            ...
            var result = await _database.ExecuteNonQueryAsync<int>(SPConstant.CREATE_NOTE, param);
            if (result.Item1 >= 1) {
