...
namespace TradiesJob.Domain.CommandHandlers {
    ...
    public sealed class JobUpdaetCommandHandler : ICommandHandler<JobUpdateCommand, AppResult> {

        private readonly IDatabase _database;
        public JobUpdaetCommandHandler(IDatabase database) {
            _database = database;
        }

        public async Task<AppResult> Handle(JobUpdateCommand command) {
            ...
            var result = await _database.ExecuteNonQueryAsync<int>(SPConstant.UPDATE_JOB, param);
            if (result.Item1 >= 1) {
